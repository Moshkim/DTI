...
//
//  EbikeSettingViewController.swift
//  E-Bike App
//
//  Created by KWANIL KIM on 6/27/17.
...
class GraphViewController: UIViewController {
    ...
    lazy var switchButton: UISwitch = {
        ...
        let button = UISwitch()
        button.isOn = true
        button.addTarget(self, action: #selector(removeDark), for: .valueChanged)
        return button
    
    }()
    
    func removeDark(sender: UISwitch) {
    
        if (sender.isOn == false) {
            
            graphView.removeFromSuperview()
            sender.isOn = true
            graphView = createDarkGraphWithWhiteBackground(CGRect(x: 0, y: 0, width: self.view.frame.width, height: self.view.frame.height/2))
            ...
            graphView.removeFromSuperview()
            sender.isOn = false
            graphView = createDarkGraph(CGRect(x: 0, y: 0, width: self.view.frame.width, height: self.view.frame.height/2))
            graphView.set(data: data, withLabels: labels)
            self.view.addSubview(graphView)
            //self.view.backgroundColor = UIColor.colorFromHex(hexString: "#333333")
        } else {
    ...
    override func viewDidLoad() {
        ...
        graphView = ScrollableGraphView(frame: CGRect(x: 0, y: 0, width: view.frame.width, height: view.frame.height/2))
        graphView = createDarkGraph(CGRect(x: 0, y: 0, width: view.frame.width, height: view.frame.height/2))
        ...
        _ = switchButton.anchor(view.centerYAnchor, left: nil, bottom: nil, right: view.rightAnchor, topConstant: 10, leftConstant: 0, bottomConstant: 0, rightConstant: 10, widthConstant: 50, heightConstant: 30)
    ...
    fileprivate func createDarkGraphWithWhiteBackground(_ frame: CGRect) -> ScrollableGraphView {
        let graphView = ScrollableGraphView(frame: frame)
        ...
        graphView.dataPointSpacing = 80
        graphView.dataPointSize = 4
        graphView.dataPointFillColor = UIColor.DTIRed()
    ...
    private func setupConstraints() {
        ...
        let topConstraint = NSLayoutConstraint(item: self.graphView, attribute: NSLayoutAttribute.top, relatedBy: NSLayoutRelation.equal, toItem: self.view, attribute: NSLayoutAttribute.top, multiplier: 1, constant: 60)
        ...
        let bottomConstraint = NSLayoutConstraint(item: self.graphView, attribute: NSLayoutAttribute.bottom, relatedBy: NSLayoutRelation.equal, toItem: self.view, attribute: NSLayoutAttribute.centerY, multiplier: 1, constant: 0)
